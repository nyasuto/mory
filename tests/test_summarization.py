...
class TestSummarizationPerformance:
    ...
    async def test_summary_generation_performance(self, mock_openai_service):
        ...
        # Arrange
        text = "Performance test text. " * 100
        ...
        # Assert
        assert summary is not None
        SummaryAssertions.assert_summary_generation_time(
            generation_time_ms, max_time_ms=100
        )  # Mock should be fast

    @pytest.mark.asyncio
    async def test_batch_summarization_performance(self, mock_openai_service):
        """Test batch summarization performance"""
        # Arrange
        texts = [f"Batch performance test text {i}. " * 20 for i in range(10)]
        ...
        # Assert
        assert len(results) == len(texts)
        # Batch processing should be reasonable even for mock
        assert batch_time_ms < 1000  # Should be under 1 second for mock
